...
def get_id_from_acc(acc):


    if acc.startswith("Prevalence"):
        acc = acc.split('|')
        var_id = acc[0].replace('Prevalence_Sequence_ID:', '')
        aro = acc[2].replace('ARO:', '')
        name = acc[1].replace("ARO_Name:", '') + "_" + var_id
    else:
        var_id = '0'
        aro = acc.split('|')[2]
        name = acc[3]

    seq_id = "{}99999{}".format(aro, var_id)

    taxid = aro

    return seq_id, taxid, name


if __name__ == '__main__':
    # conversion table tsv mapping seqIDs to taxonIDS first space
    conversion_table = []

    # taxonomy-tree \t|\t separate file mapping taxon ids to mapping taxon_ids to parents
    taxonomy_tree = []
    taxonomy_tree.append('{}\t|\t{}\t|\t{}\t|\n'.format(1, 1, "root"))

    # name table \t|\t sepeareted file mapping taxon iDS to name
    name_table = []
    ...
    seen_seq = []
    ix = 1000
    for amr_family in tqdm(glob.glob("family_fasta/*")):

        clean_name = amr_family.replace('.fna', '').replace('(', '').replace(')', '').replace("'", '').replace('"', '')

        family_id = ix
        ix += 1

        with open(amr_family) as fh:
            for record in SeqIO.parse(fh, 'fasta'):
                seq_id, taxon_id, name = get_id_from_acc(record.description)
                if seq_id not in seen_seq:
                    seen_seq.append(seq_id)

                    conversion_table.append("{}\t{}\n".format(seq_id, taxon_id))
                    taxonomy_tree.append('{}\t|\t{}\t|\t{}\t|\n'.format(taxon_id, family_id, "species"))
                    name_table.append("{}\t|\t{}\t|\t\t|\tscientific name\t|\n".format(taxon_id, name))
                    ref_sequences.append(">{}\n{}\n".format(seq_id, record.seq))
                else:
                    continue

        taxonomy_tree.append('{}\t|\t{}\t|\t{}\t|\n'.format(family_id, 1, "kingdom"))
        name_table.append("{0}\t|\t{1}\t|\t\t|\tscientific name\t|\n".format(family_id, clean_name))

    with open('card.conv', 'w') as fh:
        for line in set(conversion_table):
            fh.write(line)

    with open('card.tree', 'w') as fh:
        for line in set(taxonomy_tree):
            fh.write(line)

    with open('card.name', 'w') as fh:
        for line in set(name_table):
            fh.write(line)

    with open('card.fa', 'w') as fh:
        for line in set(ref_sequences):
            fh.write(line)
