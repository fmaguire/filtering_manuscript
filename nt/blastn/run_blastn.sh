...
makeblastdb -dbtype nucl -in ../../dbs/card_canonical_nts.fna -out card_canonical

# default word size is 3
#/usr/bin/time -o time2.tsv -a -f "%C\t%e\t%M" blastn -query test_metagenome.fna -db card_canonical -out default.out6 -outfmt 6 -num_threads 2 
### remove all but top hit (due to 'max_target_seqs' kerfuffle)
#awk '! a[$1]++' default.out6 > default.top_hits
#rm default.out6

# default word size is 3
/usr/bin/time -o time2.tsv -a -f "%C\t%e\t%M" blastn -task blastn -query test_metagenome.fna -db card_canonical -out blastn_default.out6 -outfmt 6 -num_threads 2 
# remove all but top hit (due to 'max_target_seqs' kerfuffle)
awk '! a[$1]++' blastn_default.out6 > blastn_default.top_hits
